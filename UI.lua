        ...
        function TabFunctions:Dropdown(title, list, callback)
                ...
                Choose.InputBegan:Connect(function(inp)
                    if inp.UserInputType == Enum.UserInputType.MouseButton1 then 
                        if not dropped then
                            dropped = true
                            OptionHolder:TweenSize(UDim2.new(1, 0, 0, OptionList.AbsoluteContentSize.Y + 15), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, .2, true)
                            wait(.2)
                            Dropdown:TweenSize(UDim2.new(1, -7,0, UIListLayout_69.AbsoluteContentSize.Y), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, .2, true)
                            TweenService:Create(arrow, TweenInfo.new(.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = 180}):Play()
                        else
                            dropped = false 
                            TweenService:Create(arrow, TweenInfo.new(.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = 0}):Play()
                            Dropdown:TweenSize(UDim2.new(1, -7,0, 34), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, .12, true)
                        end
                    end
                end)
